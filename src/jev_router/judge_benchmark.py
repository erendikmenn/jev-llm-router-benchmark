...
def run_judge_benchmark(
    ...
    *,
    ...
    summary = {
        ...
        / max(1, sum(row["expected_action"] != "accept" for row in rows)),
    ...
    with (output / "measurements.jsonl").open("w", encoding="utf-8") as handle:
        for row in rows:
    ...
    (output / "REPORT_TR.md").write_text(_report(summary), encoding="utf-8")
    return rows, summary
