...
- Koşu: `20260919T165242Z`
- Mod: `fixture`
- Örnek sayısı: 20
- Jev eşiği: 0.680
- Jev güçlü model seçim oranı: %30.0
...
|---|---:|---:|---:|---:|---:|---:|---:|
| always_strong | 0.988 | +0.00 yp | [+0.00, +0.00] | %100.0 | $0.011480 | %0.0 | 307.0 / 335.1 |
| always_cheap | 0.450 | -53.75 yp | [-75.00, -31.25] | %0.0 | $0.000530 | %95.4 | 162.0 / 190.1 |
| rule | 0.500 | -48.75 yp | [-70.00, -27.50] | %10.0 | $0.002642 | %77.0 | 164.5 / 293.9 |
| random_matched | 0.600 | -38.75 yp | [-60.00, -18.75] | %25.0 | $0.002506 | %78.2 | 169.0 / 309.0 |
| jev | 0.650 | -33.75 yp | [-55.00, -15.00] | %30.0 | $0.005476 | %52.3 | 191.5 / 360.0 |

## Önceden tanımlı hedef

Hedef, güçlü baseline'a göre toplam başarı/kalite kaybını en fazla 2 yüzde puanında tutarken maliyeti azaltmaktı. Bu koşuda ölçülen kayıp **33.75 yüzde puanı**. Küçük ve sentetik fixture kümesi kalite korumasını kanıtlamak için yeterli değildir; güven aralığı ve alt gruplar kararın parçasıdır.

## Hata analizi

- Jev ucuz modeli seçtiği halde tam başarı sağlanamayan örnek: 8
- Ucuz model aynı kaliteyi sağlayabilecekken güçlü model seçilen örnek: 2
- Fallback oranı: %0.0
- Hata oranı: %0.0
- Fixture iş yükünde Jev ek maliyetini karşılamak için gereken asgari ucuz-model oranı: %1.8
...
- `demo-012` — general_qa / en, kalite 0.00, kural `strong_probability_lt_0.680`
- `demo-023` — extraction_classification / tr, kalite 0.50, kural `strong_probability_lt_0.680`
- `demo-024` — extraction_classification / tr, kalite 0.00, kural `strong_probability_lt_0.680`
- `demo-031` — general_qa / tr, kalite 0.00, kural `strong_probability_lt_0.680`
- `demo-032` — general_qa / tr, kalite 0.00, kural `strong_probability_lt_0.680`
...
|---|---:|---:|---:|---:|---:|
| en | 0.975 | 0.450 | 0.450 | 0.650 | 0.650 |
| tr | 1.000 | 0.450 | 0.550 | 0.550 | 0.650 |

### Görev grubu

| Grup | always_strong | always_cheap | rule | random_matched | jev |
|---|---:|---:|---:|---:|---:|
| coding | 1.000 | 0.500 | 0.750 | 0.500 | 1.000 |
| extraction_classification | 1.000 | 0.250 | 0.250 | 0.750 | 0.250 |
| general_qa | 1.000 | 0.000 | 0.000 | 0.250 | 0.000 |
| multi_step_reasoning | 1.000 | 0.500 | 0.500 | 0.500 | 1.000 |
| translation_summary | 0.938 | 1.000 | 1.000 | 1.000 | 1.000 |
